...
class TimeSeriesDatasetCreator:
    ...
    def merge_features_to_dataset(self):
        print('Vytváram finálny dataset podľa vybraných atribútov.')
        try:
            protocol_features = json.load(open(const.SELECTED_FEATURES_JSON.format(self.window_size)))
            Path(const.EXTRACTED_DATASETS_PATH.format(self.window_size)).mkdir(parents=True, exist_ok=True)
        except:
            print('Súbor (.json) s vybranými atribútmi nebol nájdený !')
            return
        
        time = pd.read_csv(const.TS_DATASET_BY_CATEGORY_PATH.format(self.window_size, 'Normal', 'all'), usecols = [const.TIME])
        for attack_type in const.ATTACK_CATEGORIES:
            data = pd.DataFrame(time)
            for protocol in protocol_features:
                path = const.TS_DATASET_BY_CATEGORY_PATH.format(self.window_size, attack_type, protocol)
                benign_protocol_data = pd.read_csv(
                                            const.TS_DATASET_BY_CATEGORY_PATH.format(self.window_size, 'Normal', protocol), 
                                            usecols = protocol_features[protocol]
                                    )
                combined_data = self.handle_outliers(benign_protocol_data)   # At this point it contains only benign traffic
            ...
            data.to_csv(const.EXTRACTED_ATTACK_CAT_DATASET_PATH.format(self.window_size, attack_type), index=False)
        
        print('Vytvorenie dokončené !')
        self.plot_merged_dataset()
    ...
    def remove_outliers(self, data, upper):
    ...
    def plot_merged_dataset(self):
        print('Ukladám grafy ...')
        for attack in const.ATTACK_CATEGORIES:
            path = const.EXTRACTED_DATASETS_PLOTS_PATH.format(self.window_size, attack)
            Path(path).mkdir(parents=True, exist_ok=True)
            data = pd.read_csv(const.EXTRACTED_ATTACK_CAT_DATASET_PATH.format(self.window_size, attack))
